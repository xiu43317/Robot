...
class Camera_Servo():
	...
	def act(self, act = 0):
		...
		elif self.pulse < robot.servoMin:
...
#	CLOUD_IP = '172.20.10.3'
	...
	def movement_link(arg):
		if arg == 'c':
			...
			com = "wget http://%s:%d/?action=snapshot -O GPIO.output.jpg" % getip(), CAM_PORT
	...
	def distance():
		...
		for i in [ 0, 1 ]:
			us[i].dist_init()
		while us[0].dist_loop_do() or us[0].dist_loop_do():
			pass
		return ("%.5f,%.5f" % us[0].loop, us[1].loop)
		'''

	@app.route("/upload")
	def upload():
		register_openers()
		datagen, headers = multipart_encode({"image1": open("GPIO.output.jpg", 'rb')})
